...
async function initierPaiementCinetPay(plan, setStatus) {
  ...
  const payload = {
    ...
    lang: "fr",
    metadata: JSON.stringify({ plan: plan.id }),
  };
  ...
  try {
    ...
    if (data.code === "201" && data.data?.payment_url) {
      ...
      setStatus({ loading: false, error: data.message || "Erreur de paiement. Contactez-nous sur WhatsApp.", success: false });
    }
  } catch {
    setStatus({ loading: false, error: "Connexion impossible. Vérifiez votre internet ou contactez-nous sur WhatsApp.", success: false });
  }
}
  ...
  return (
    <div style={{ border: plan.featured ? "2px solid #3b82f6" : "1px solid #e5e5e5", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
        <button
          onClick={() => initierPaiementCinetPay(plan, setStatus)}
        ...
        >
          {status.loading ? "⏳ Connexion..." : "📱 Payer MTN MoMo / Orange Money"}
        </button>
